...
function timeElapse(c){
	...
	if(d<10){d="0"+d}
		f=f%60;
	if(f<10){f="0"+f}
	var a='<span class="digit">'+g+'</span> days <span class="digit">'+b+'</span> hours <span class="digit">'+d+
	'</span> minutes <span class="digit">'+f+"</span> seconds";$("#elapseClock").html(a)}
	function timeElapse1(c){
	var e=Date();
	var f=(Date.parse(e)-Date.parse(c))/1000;
	var g=Math.floor(f/(3600*24)+31);
	f=f%(3600*24);
	var b=Math.floor(f/3600);
	if(b<10){b="0"+b}
		f=f%3600;
